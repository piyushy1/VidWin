        ...
        try:
            ...
            if temp_block == []:
                ...
                if new_frame_data[0] == "I":
                    ...
                    random_acc_required = 0.7
                    dist = get_frame_distance(new_frame_data[0], temp_block[0][0])
                    if dist < 0.8:
                        # key_frame = temp_block[0][0]
                        # key_frame_100 = cv2.resize(key_frame, (100, 100))
                        # key_frame_200 = cv2.resize(key_frame, (200, 200))
                        # preds_100 = batch_prediction(np.stack([key_frame_100], axis=0), model)
                        # preds_200 = batch_prediction(np.stack([key_frame_200], axis=0), model)
                        # print(preds_100, " : ", preds_200)
                        
                        # if preds_100 > random_acc_required:
                        #     tf.images.resize(images, (100, 100))
                        # else:
                        #     tf.images.resize(images, (200, 200))
...
def intialize_timewindow(publisher_id, time_duration):
    ...
    # print(elasped_time, "s for frame by frame processing")
        ...
        try:
            ...
            key_frame = new_batch[0][0]
            processed_batch = []

            key_frame_150 = cv2.resize(key_frame, (150, 150))
            preds_150 = batch_prediction(np.stack([key_frame_150], axis=0), model)
            #print('Accuracy: ', preds_150) 

            #print(preds_100, " : ", preds_150)            
            # prob = random.randint(0,100)/100

            if preds_150[0][1][1] >= random_acc_required:  # if car acc >= required then resize
                for i in new_batch:
                    processed_batch.append(cv2.resize(i[0], (150, 150)))
            else:
                key_frame_300 = cv2.resize(key_frame, (300, 300))
                preds_300 = batch_prediction(np.stack([key_frame_300], axis=0), model)
                if preds_300[0][1][1] >= random_acc_required:
                    for i in new_batch:
                        processed_batch.append(cv2.resize(i[0], (300, 300)))
                else:
                    for i in new_batch:
    ...
    end_time = datetime.now()
    elasped_time = (end_time - start_time).total_seconds()
    print(elasped_time, "s for key frame + micro batch processing")
